...
#### Sprint 1（Week 1-2）：核心框架 + 构建态基础

```
Sprint 1 目标：ALE 核心框架 + 构建态意图理解基础

Week 1: 核心框架 + 意图理解引擎
├── @ale/core 插件开发
│   ├── [4h] 插件骨架、生命周期
│   ├── [4h] 审计日志基础设施
│   ├── [4h] 上下文中间件（correlationId, spaceId）
│   └── [4h] 通用工具函数
│
├── @ale/intent-engine 插件开发（构建态核心）⭐
│   ├── [8h] LLM 集成封装（OpenAI/Claude）
│   ├── [8h] 意图解析服务（Intent Parser）
│   ├── [8h] 方案生成服务（Proposal Generator）
│   └── [8h] 意图→对象/关系/状态机转换
│
├── @ale/ontology 插件开发
│   ├── [4h] ontology_objects 表设计与实现
│   ├── [4h] ontology_relations 表设计与实现
│   ├── [4h] OntologyRegistry 服务核心方法
│   └── [4h] 与 Collection Manager 集成

Week 2: 门禁引擎 + 响应式视图
├── @ale/gate-engine 插件开发
│   ├── [4h] BaseGate 抽象类设计
│   ├── [4h] G1_STRUCTURAL 门禁实现
│   ├── [4h] G3_EVIDENCE 门禁实现
│   ├── [4h] G6_EXECUTION 门禁实现
│   ├── [4h] G7_EVALUATION 门禁实现
│   └── [4h] GateEngine 服务整合
│
├── @ale/dynamic-view 插件开发（构建态核心）⭐
│   ├── [8h] 响应式视图框架设计
│   ├── [8h] 对象结构预览组件
│   ├── [8h] 状态机预览组件
│   └── [8h] 方案→视图实时更新机制
│
├── @ale/changeset 插件开发
│   ├── [4h] changesets 表设计与实现
│   ├── [4h] ChangeSetService 核心方法
│   ├── [4h] 与 GateEngine 集成
│   └── [4h] 审批流程基础实现

Sprint 1 交付物:
- [ ] @ale/core v0.1.0
- [ ] @ale/intent-engine v0.1.0 ⭐ 构建态核心
- [ ] @ale/dynamic-view v0.1.0 ⭐ 构建态核心
- [ ] @ale/ontology v0.1.0
...
#### Sprint 3（Week 5-6）：双态 UI 与闭环

```
Sprint 3 目标：构建态 + 运行态端到端闭环可演示

Week 5:
├── 构建态 UI（ALE Studio）⭐
│   ├── [8h] 对话式交互界面（意图输入 + AI 响应）
│   ├── [8h] 动态预览面板（对象结构/状态机/UI效果）
│   ├── [8h] 变更摘要 + 门禁预检展示
│   ├── [8h] ChangeSet 确认 + 提交流程
│   └── [8h] 迭代调整交互（修改意图→更新预览）
│
├── 运行态 UI（自动生成）
│   ├── [8h] 处置单列表页（从本体生成）
...
| 功能点 | 验收标准 | 优先级 |
|-------|---------|-------|
| 意图理解 | 用户自然语言描述需求，AI 正确识别对象/关系/状态机 | P0 |
| 响应式预览 | 意图变化后 3 秒内预览视图更新 | P0 |
| 多维度预览 | 支持对象结构、状态机、影响分析三种预览 | P0 |
| 迭代调整 | 用户可连续对话调整方案，视图持续更新 | P0 |
| ChangeSet 生成 | 方案确认后正确生成 ChangeSet | P0 |
| 门禁预检 | 构建阶段展示门禁预检结果 | P1 |
